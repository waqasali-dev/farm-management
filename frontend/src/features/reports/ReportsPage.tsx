  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      if (reportType === 'summary') {
        res = await api.getSummaryReport(activeFlock.id);
      } else if (reportType === 'mortality') {
        res = await api.getMortalityReport(activeFlock.id);
      } else if (reportType === 'feed') {
        res = await api.getFeedReport(activeFlock.id);
      } else if (reportType === 'eggs') {
        res = await api.getEggsReport(activeFlock.id);
      }
      setData(res);
    } catch (err: any) {
